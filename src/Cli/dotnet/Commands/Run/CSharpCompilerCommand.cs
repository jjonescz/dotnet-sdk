...
internal sealed partial class CSharpCompilerCommand
{
    ...
    private static string NetCoreAppHostPackVersion => field ??= ResolvePackVersion($"Microsoft.NETCore.App.Host.{RuntimeInformation.RuntimeIdentifier}");
#else
    ...
    private static string NetCoreAppRefPackVersion => RuntimeVersion;
    private static string NetCoreAppHostPackVersion => RuntimeVersion;
#endif
    ...
    public int Execute(out bool fallbackToNormalBuild)
    {
        ...
        if (exitCode == 0 &&
            ...
            TryGetOutputFile(CscArguments, out var outputFile))
        {
        ...
        {
            string csharpCompilerAssembly = Path.Combine(ClientDirectory, "Microsoft.CodeAnalysis.CSharp.dll");
            string? productVersion = FileVersionInfo.GetVersionInfo(csharpCompilerAssembly).ProductVersion;
            int hashStart = productVersion?.IndexOf('+') ?? -1;

            return hashStart >= 0 && hashStart + 1 < productVersion!.Length
                ? productVersion[(hashStart + 1)..]
                : throw new InvalidOperationException($"Could not find compiler commit hash in '{csharpCompilerAssembly}'.");
        }
    ...
    private void PrepareAuxiliaryFiles(out string rspPath)
    {
        ...
        if (ShouldEmit(apphostTarget))
        {
            ...
            var apphostSource = Path.Join(DotNetRootPath, "packs", $"Microsoft.NETCore.App.Host.{rid}", NetCoreAppHostPackVersion, "runtimes", rid, "native", $"apphost{FileNameSuffixes.CurrentPlatform.Exe}");
