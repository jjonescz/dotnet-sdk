...
#pragma warning disable RSEXPERIMENTAL003 // 'SyntaxTokenParser' is experimental
    {
        ...
        foreach (var trivia in result.Token.LeadingTrivia)
        {
            ...
            else if (trivia.IsKind(SyntaxKind.IgnoredDirectiveTrivia))
            {
                ...
                var parsed = FileBasedProgramDirective.TryParse(locationInfo, span, name.ToString(), value.ToString(), diagnostics);
                if (parsed != null)
                {
        ...
        if (reportAllErrors)
        {
            ...
            do
            {
                ...
                foreach (var trivia in result.Token.LeadingTrivia)
                {
                    reportErrorFor(file, trivia, diagnostics);
                }
        ...
        static void reportErrorFor(SourceFile file, SyntaxTrivia trivia, ImmutableArray<Diagnostic>.Builder diagnostics)
        {
            if (trivia.ContainsDiagnostics && trivia.IsKind(SyntaxKind.IgnoredDirectiveTrivia))
            {
                ...
                diagnostics.Add(Diagnostic.Create(FileBasedProgramDiagnostics.CannotConvertDirective, location));
            }
        }
    }
...
{
    internal const string FileBasedProgramProject = "RSEXPERIMENTAL006";
    internal const string FileBasedProgramProject_Url = "https://github.com/dotnet/roslyn/issues/78159";
}
